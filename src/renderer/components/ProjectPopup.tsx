...
const ProjectPopup: React.FC<ProjectPopupProps> = ({
  ...
  // Step 1 = select project, Step 2 = select running program
    ...
    return (
      <div style={cardStyle}>
        ...
        <div style={footerStyle}>
          ...
          <button
            onClick={() => { if (selectedProjectId) setStep(2); }}
    ...
    <div style={cardStyle}>
      <div style={headerStyle}>
        <button onClick={() => setStep(1)} style={{ background: 'none', border: 'none', color: '#718096', cursor: 'pointer', fontSize: '13px', padding: 0, marginBottom: '4px' }}>
          ← {selectedProject?.name}{selectedProject?.subproject && ` › ${selectedProject.subproject}`}
        </button>
      ...
      {/* App list */}
      <div style={{ padding: '8px 16px', maxHeight: '220px', overflowY: 'auto' }}>
        {loadingApps ? (
          <div style={{ textAlign: 'center', padding: '20px', color: '#4A5568', fontSize: '13px' }}>Loading...</div>
        ) : filteredApps.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '20px', color: '#4A5568', fontSize: '13px' }}>No running programs detected</div>
        ) : (
      ...
      <div style={footerStyle}>
        ...
        <button onClick={handleConfirm} disabled={!selectedApp}
